...
3. **Numerator** = sum of patients having all 8 physiological targets set (see the section above on VALIDITY)
4. **Denominator** = total number of current patients (occupied beds) at 13:30 hours
6. Calculate daily percentage for each unit, (exceptionally) treating T03 as Northside & Southside (e.g. T03N, T03S, T06, GWB & WMS units)
7. Aggregate the daily percentages into a **-weekly mean percentage_** for each of these respective units (T06, GWB & WMS)
8. Aggregate the daily percentages into a **_weekly mean percentage_** for T03 Northside & T03 Southside producing one value for the unit T03
9. SPC data point = weekly aggregated mean of numerator/denominator  as percentage
10. Plot an SPC chart for each respective unit: y-axis = weekly percentage; x-axis = time


### n-number for process limits

 n-number for process limits = the aggregated denominator (ie. the total sum of the number of inpatients (occupied beds) at 13:30 hours taken on each day of the calendar week (i.e. weekly aggregate of point 4 above)

**Tooltip display = process limit n-number and labelled as 'eligible patients' 

> [!NOTE]
> As of 2025-03-14 this is correct.

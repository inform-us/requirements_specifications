...
For those that include discharged/moved patients, they should use all data produced by patients _on the current unit_ in the last 24 hours. For example, let's consider the tiles on the T03 dashboard. Let's imagine a patient has been on T03 for multiple days in a specific bed, before being moved 12 hours ago to another ward. In this scenario we would use all data generated by the patient on T03 in the last 24 hours (so from 12-24 hours ago), but ignore any data generated while they are off the unit (0-12 hours ago). No data generated outside of the unit should be included in any tile calculations (Note: airway is an exception to this - see [the airway rules](./Airway%20Plan.md)).
...
Note: during processing, you will need to consider a larger time window for most metrics to ensure calculated statistics are correct (see extended hours)
